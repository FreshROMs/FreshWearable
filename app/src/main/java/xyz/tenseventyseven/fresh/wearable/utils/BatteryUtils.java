...
public class BatteryUtils {
    public static List<String> formatDeviceBattery(Context context, GBDevice device, boolean hasStatus, boolean includeCase) {
        ...
        boolean isBuds = coordinator.getGeneralDeviceType() == DeviceCoordinator.GeneralDeviceType.EARBUDS;
    ...
    private static void formatEarbudsBattery(GBDevice device, int batteryCount, List<String> formatted, Context context, boolean includeCase) {
        ...
        if (batteryCount == 3) {
            if (shouldCombineBudsBattery(device)) {
                ...
                if (includeCase) {
                    formatted.add(context.getString(R.string.device_buds_battery_status_combined, battery, batteryCase));
                } else {
                    formatted.add(context.getString(R.string.device_buds_battery_status_combined_no_case, battery));
                }
            } else {
                if (includeCase) {
                    formatted.add(context.getString(R.string.device_buds_battery_status_separate, batteryLeft, batteryRight, batteryCase));
                } else {
                    formatted.add(context.getString(R.string.device_buds_battery_status_separate_no_case, batteryLeft, batteryRight));
                }
            }
        } else if (batteryCount == 2) {
            if (includeCase) {
                formatted.add(context.getString(R.string.device_buds_battery_status_single_bud, batteryLeft, batteryCase));
            } else {
                formatted.add(context.getString(R.string.device_battery_status_single, batteryLeft));
            }
        }
    }
    ...
    private static void formatSingleBattery(GBDevice device, boolean hasStatus, List<String> formatted, Context context) {
        ...
        formatted.add(context.getString(R.string.device_battery_status_single, battery));
