...
public class BluetoothConnectReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        final GBDevice gbDevice = WearableApplication.app().getDeviceManager().getDeviceByAddress(device.getAddress());
        if (gbDevice == null) {
            LOG.info("Connected device {} unknown", device.getAddress());
            return;
        }
        final SharedPreferences deviceSpecificPreferences = WearableApplication.getDeviceSpecificSharedPrefs(gbDevice.getAddress());
        ...
        WearableApplication.deviceService(gbDevice).connect();
    }
}
