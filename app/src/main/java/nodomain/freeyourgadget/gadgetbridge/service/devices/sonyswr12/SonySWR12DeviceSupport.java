...
public class SonySWR12DeviceSupport extends AbstractBTLEDeviceSupport {
    ...
    public void onSetAlarms(ArrayList<? extends Alarm> alarms) {
        try {
            ...
            int prefInterval = Integer.valueOf(WearableApplication.getDeviceSpecificSharedPrefs(gbDevice.getAddress())
        ...
        try {
            switch (config) {
                case DeviceSettingsPreferenceConst.PREF_SONYSWR12_STAMINA: {
                    ...
                    int status = WearableApplication.getDeviceSpecificSharedPrefs(gbDevice.getAddress()).getBoolean(config, false) ? 1 : 0;
                ...
                case DeviceSettingsPreferenceConst.PREF_SONYSWR12_LOW_VIBRATION: {
                    boolean isEnabled = WearableApplication.getDeviceSpecificSharedPrefs(gbDevice.getAddress()).getBoolean(config, false);
                    TransactionBuilder builder = performInitialized(config);
