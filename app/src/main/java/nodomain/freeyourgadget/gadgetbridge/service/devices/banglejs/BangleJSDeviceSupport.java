...
public class BangleJSDeviceSupport extends AbstractBTLEDeviceSupport {
    ...
    private void stopGlobalUartReceiver(){
        if(globalUartReceiver != null){
            Application.getContext().unregisterReceiver(globalUartReceiver); // remove uart.tx intent listener
        }
    }
    ...
    private void registerGlobalIntents() {
            ...
            public void onReceive(Context context, Intent intent) {
                switch (intent.getAction()) {
                    case BANGLE_ACTION_UART_TX: {
                         ...
                         */
                        Prefs devicePrefs = new Prefs(Application.getDeviceSpecificSharedPrefs(gbDevice.getAddress()));
        ...
        ContextCompat.registerReceiver(Application.getContext(), globalUartReceiver, commandFilter, ContextCompat.RECEIVER_EXPORTED);
    }
    ...
    public void onSetGpsLocation(final Location location) {
        if (!Application.getPrefs().getBoolean("use_updated_location_if_available", false)) return;
    ...
    public String renderUnicodeAsImage(String txt) {
        ...
        if (!devicePrefs.getBoolean(PREF_BANGLEJS_TEXT_BITMAP, false))
            return EmojiConverter.convertUnicodeEmojiToAscii(txt, Application.getContext());
    ...
    protected long getLastSuccessfulSyncTime() {
        long timeStampMillis = Application.getDeviceSpecificSharedPrefs(getDevice().getAddress()).getLong(getLastSyncTimeKey(), 0);
