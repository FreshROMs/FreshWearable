...
public abstract class Casio2C2DSupport extends CasioSupport {
    ...
    public class FeatureCache {
        ...
        void load() {
            values = new HashMap();
            Set<String> serialized = WearableApplication.getDeviceSpecificSharedPrefs(gbDevice.getAddress()).getStringSet("casio_features_current_values", new HashSet());
            if (serialized == null)
                return;
    ...
    void initializeDeviceSettings(TransactionBuilder builder) {
        ...
        requestFeatures(builder, deviceSettingFeatures, responses -> {
            ...
            SharedPreferences.Editor editor = WearableApplication.getDeviceSpecificSharedPrefs(gbDevice.getAddress()).edit();
            for (DeviceSetting ds: deviceSettings) {
