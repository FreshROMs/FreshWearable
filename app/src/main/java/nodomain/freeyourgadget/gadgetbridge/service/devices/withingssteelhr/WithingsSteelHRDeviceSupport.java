...
public class WithingsSteelHRDeviceSupport extends AbstractBTLEDeviceSupport {
    ...
    private static Prefs prefs = Application.getPrefs();
    ...
    private void saveLastSyncTimestamp(@NonNull long timestamp) {
        SharedPreferences.Editor editor = Application.getDeviceSpecificSharedPrefs(getDevice().getAddress()).edit();
        editor.putLong(LAST_ACTIVITY_SYNC, timestamp);
        editor.apply();
    }

    private long getLastSyncTimestamp() {
        SharedPreferences settings = Application.getDeviceSpecificSharedPrefs(getDevice().getAddress());
    ...
    private Locale getLocale() {
        String localeString = Application.getDeviceSpecificSharedPrefs(gbDevice.getAddress())
    ...
    private short getUnit() {
        String units = prefs.getString(SettingsActivity.PREF_MEASUREMENT_SYSTEM, Application.getContext().getString(R.string.p_unit_metric));

        if (units.equals(Application.getContext().getString(R.string.p_unit_metric))) {
